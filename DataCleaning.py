...
for i in tqdm(range(len(ETF_list))):
    ...
    ETF_dfs[i].to_csv("/Users/anikmalik/Desktop/Fin-GAN-main/data/"+ETF+".csv")
...
for i in tqdm(range(len(Stock_list))):
    ...
    Stock_dfs[i].to_csv("/Users/anikmalik/Desktop/Fin-GAN-main/data/"+Ticker+".csv")
...
#dataloc = "/Users/anikmalik/Desktop/Fin-GAN-main/data/"
#stock = 'AMZN'
#etf = 'XLY'
#e_ret, sret, etfret = FinGAN.excessreturns(dataloc, stock, etf, plotcheck = False)
#plt.figure()
...
# Your modified function now returns (excessret, dates_dt)
e_ret, dates_dt = FinGAN.excessreturns(dataloc, stock, etf, plotcheck=False)

# (Optional) If you ALSO changed it to return sret and etfret, use the next line instead:
# e_ret, sret, etfret, dates_dt = FinGAN.excessreturns(dataloc, stock, etf, plotcheck=False)

# --- mismatch guard (keeps things running even if one vector is off by 1) ---
n = min(len(dates_dt), len(e_ret))
dates_dt = dates_dt.iloc[:n] if hasattr(dates_dt, "iloc") else dates_dt[:n]
e_ret = e_ret[:n]
...
etflistloc = "/Users/anikmalik/Desktop/Fin-GAN-main/stocks-etfs-list.csv"
ticker = "PEP"
etf = FinGAN.ETF_find(etflistloc,ticker)
e_ret, sret = FinGAN.excessreturns_closeonly(dataloc, ticker, etf)
train_data, val_data, test_data, dates_dt  = FinGAN.split_train_val_test(ticker, dataloc, etflistloc, tr = 0.8, vl = 0.1, h = 1, l = 10, pred = 1, plotcheck=False)
